            ...
            if opcode == 1:  # addition
                ...
                if self.phase_input_set:
                    if self.input is None:
                        break
                    self.program[parameter1] = self.input
                    self.input = None
                else:
                    self.program[parameter1] = self.phase_setting
...
for phase_settings in all_feedback_mode_phase_settings():
    output = 0
    previous_last_output = None

    amplifier_programs = [
        AmplifierIntcode(program.copy(), None, int(phase_settings[0])),
        AmplifierIntcode(program.copy(), None, int(phase_settings[1])),
        AmplifierIntcode(program.copy(), None, int(phase_settings[2])),
        AmplifierIntcode(program.copy(), None, int(phase_settings[3])),
        AmplifierIntcode(program.copy(), None, int(phase_settings[4])),
    ]
    ...
    while in_progress:
        for i in range(len(phase_settings)):
            ...
            if output and i == 4:
                previous_last_output = output

            if amplifier.halted and i == 4:
                in_progress = False
                break

    if previous_last_output > max_output:
        max_output = previous_last_output
        max_output_phase_settings = phase_settings
