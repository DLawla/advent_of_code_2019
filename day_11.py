...
class Robot():
    def __init__(self, program):
        ...
        self.visited_coordinates_colors = []
        self.orientation = 0
    ...
    def perform_one_tick(self):
        ...
        if outputs.__len__() == 2:
            ...
            self.visited_coordinates += [self.coordinates]
            self.visited_coordinates_colors += [painted_color]
            self.perform_movement(direction_to_turn)

    def color_of_coordinate(self):
        matching_coordinates = [c for c in self.visited_coordinates if c == self.coordinate]
        if matching_coordinates.__len__() == 0:
            return 0
        index = self.visited_coordinates.index(matching_coordinates[0])
        self.visited_coordinates_colors[index]
...
while not done:
    ...
    if robot.intcode.halted:
        done = True

print(robot.visited_coordinates)
print(robot.visited_coordinates_colors)